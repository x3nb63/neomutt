...
#define HELP_CACHE_DOCLIST 1 /* whether to cache the DocList between hbox_open calls */
#define HELP_FHDR_MAXLINES -1 /* max help file header lines to read (N < 0 means all) */
#define HELP_LINK_CHAPTERS 0 /* whether to link all help chapter upwards to root box */

static bool __Backup_HTS; /* used to restore $hide_thread_subject on hbox_close() */
...
{
  const struct Header *h1 = *(const struct Header **) a;
  const struct Header *h2 = *(const struct Header **) b;
  const HDType t1 = ((const HDMeta *) h1->data)->type;
  const HDType t2 = ((const HDMeta *) h2->data)->type;
 ...
 */
static void help_doc_meta_free(struct Header *hdr)
{
  HDMeta *meta = ((HDMeta *) hdr->data);
...
{
  struct Header *hdoc = ((struct Header *) *item);

  mutt_header_free(&hdoc);
  FREE(hdoc);
}
...
{
  const char *bfn = mutt_file_basename(NONULL(file));
                         ...
                         int (*gather)(HelpList **, const char *), HelpList **items)
{
  ...
  while (1)
  {
    ...
    mutt_file_concat_path(abspath, curpath, np, sizeof(abspath));
...
static void *help_doc_clone(const void *item)
{
  ...
  struct Header *src = (struct Header *) item;
  struct Header *dup = mutt_header_new();
  /* struct Header */
  ...
  /* struct Header::data (custom metadata) */
  dup->data = help_doc_meta_clone(src->data);
  dup->free_cb = help_doc_meta_free;
  /* struct Body */
 ...
 */
static struct Header *help_doc_from(const char *file)
{
  HDType type = HDT_NONE;
  ...
  const char *bfn = mutt_file_basename(file);
  const char *pdn = mutt_file_basename(mutt_file_dirname(file));
  ...
  struct Header *hdoc = mutt_header_new();
  /* struct Header */
  ...
  /* struct Header::data (custom metadata) */
  hdoc->data = meta;
  hdoc->free_cb = help_doc_meta_free;
  /* struct Body */
  hdoc->content = mutt_body_new();
  hdoc->content->disposition = DISPINLINE;
  hdoc->content->encoding = ENC8BIT;
  hdoc->content->length = -1;
  hdoc->content->subtype = mutt_str_strdup("plain");
  hdoc->content->type = TYPETEXT;
  /* struct Envelope */
  hdoc->env = mutt_env_new();
  hdoc->env->from = mutt_addr_parse_list(NULL, "Richard Russon <rich@flatcap.org>");
  hdoc->env->message_id = help_doc_msg_id(tm);
  FREE(tm);
  hdoc->env->organization = mutt_str_strdup("NeoMutt");
  hdoc->env->subject = help_doc_subject(fhdr, sbj, "[%s]: %s", "title", "description", NULL);
...
{
  help_list_new_append(list, sizeof(struct Header *), help_doc_from(path));
 ...
 */
static void help_doc_uplink(const struct Header *target, const struct Header *source)
{
  if (!target || !source)
    return;
...
static void help_dir_read(const char *path)
{
  ...
  struct Header *hdoc_top = help_list_get(list, 0, NULL), *hdoc_cur = NULL;
  HDType hdoc_top_type = ((HDMeta *) hdoc_top->data)->type;

  /* uplink a help chapter/section top node */
  if (hdoc_top_type & HDT_CHAPTER)
  {
    if (HELP_LINK_CHAPTERS != 0)
      help_doc_uplink(help_list_get(DocList, 0, NULL), hdoc_top);

    UpLink = DocList->size;
  }
  else if (hdoc_top_type & HDT_SECTION)
    help_doc_uplink(help_list_get(DocList, UpLink, NULL), hdoc_top);
  else
    UpLink = 0;

  hdoc_top->index = DocList->size;
  help_list_append(DocList, hdoc_top);

  /* link remaining docs to first list item */
  for (size_t i = 1; i < list->size; i++)
  {
    hdoc_cur = help_list_get(list, i, NULL);
    help_doc_uplink(hdoc_top, hdoc_cur);

    hdoc_cur->index = DocList->size;
    help_list_append(DocList, hdoc_cur);
  }
}
...
int help_doclist_init(void)
{
  ...
  help_doclist_free();
  DocList = help_list_new(sizeof(struct Header));
...
static int help_doclist_parse(struct Context *ctx)
{
  ...
  ctx->hdrs =
      (struct Header **) (help_list_clone(DocList, true, help_doc_clone))->data;
  ctx->msgcount = ctx->hdrmax = DocList->size;
  ...
  mutt_str_replace(&ctx->realpath, mutt_str_strdup(HelpDocDir));

  /* check (none strict) what the user wants to see */
  const char *request = help_path_transpose(ctx->path, false);
  ctx->hdrs[0]->read = false;
  if (request)
  {
    mutt_str_replace(&ctx->path, help_path_transpose(request, false)); /* just sanitise */
    request += mutt_str_strlen(HelpDocDir) + 1;
    for (size_t i = 0; i < ctx->msgcount; i++)
    { /* TODO: prioritise folder (chapter/section) over root file names */
      if (mutt_str_strncmp(ctx->hdrs[i]->path, request, mutt_str_strlen(request)) == 0)
      {
        ctx->hdrs[0]->read = true;
...
static int hbox_open(struct Context *ctx)
{
  mutt_debug(1, "entering hbox_open\n");
...
static int hbox_open_append(struct Context *ctx, int flags)
{
  mutt_debug(1, "entering hbox_open_append\n");

  return -1;
}

static int hbox_check(struct Context *ctx, int *index_hint)
{
  mutt_debug(1, "entering hbox_check\n");

  return 0;
}

static int hbox_sync(struct Context *ctx, int *index_hint)
{
  mutt_debug(1, "entering hbox_sync\n");

  return 0;
}

static int hbox_close(struct Context *ctx)
{
  mutt_debug(1, "entering hbox_close\n");
...
static int hdoc_open(struct Context *ctx, struct Message *msg, int msgno)
{
  mutt_debug(1, "entering hdoc_open: %d, %s\n", msgno, ctx->hdrs[msgno]->env->subject);

  char path[PATH_MAX];
  snprintf(path, sizeof(path), "%s/%s", ctx->realpath, ctx->hdrs[msgno]->path);

  ctx->hdrs[msgno]->read = true; /* reset a probably previously set unread status */
...
static int hdoc_open_new(struct Context *ctx, struct Message *msg, struct Header *hdr)
{
  mutt_debug(1, "entering hdoc_open_new\n");

  return -1;
}

static int hdoc_commit(struct Context *ctx, struct Message *msg)
{
  mutt_debug(1, "entering hdoc_commit\n");

  return -1;
}

static int hdoc_close(struct Context *ctx, struct Message *msg)
{
  mutt_debug(1, "entering hdoc_close\n");
...
static int hdoc_tags_edit(struct Context *ctx, const char *tags, char *buf, size_t buflen)
{
  mutt_debug(1, "entering hdoc_tags_edit\n");

  return -1;
}

static int hdoc_tags_commit(struct Context *msg, struct Header *hdr, char *buf)
{
  mutt_debug(1, "entering hdoc_tags_commit\n");
...
struct MxOps mx_help_ops = {
  .mbox_open        = hbox_open,
  .mbox_open_append = hbox_open_append,
  .mbox_check       = hbox_check,
  .mbox_sync        = hbox_sync,
  .mbox_close       = hbox_close,
  .msg_open         = hdoc_open,
  .msg_open_new     = hdoc_open_new,
  .msg_commit       = hdoc_commit,
  .msg_close        = hdoc_close,
  .tags_edit        = hdoc_tags_edit,
  .tags_commit      = hdoc_tags_commit,
};
// clang-format on
